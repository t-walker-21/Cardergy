...
function scene:create( event )
	...
	local function onAddress(event)
		...
		elseif ("editing" == event.phase) then
			...
			validateInput()
		elseif ("submitted" == event.phase) then
			if (not(string.match(addressField.text, "^[%d]+ .+$"))) then
				----sceneGroup.alpha = 0.5
	...
	local function onCity(event)
		...
		elseif ("editing" == event.phase) then
			...
			validateInput()
		elseif ("submitted" == event.phase) then
			if (string.find(cityField.text, "[^%a%.- ]") ~= nil) then
				--sceneGroup.alpha = 0.5
	...
	local function onZip(event)
		...
		elseif ("editing" == event.phase) then
			...
			validateInput()
		elseif ("submitted" == event.phase) then
			if (string.find(zipField.text, "[^%d]") ~= nil) then
				--sceneGroup.alpha = 0.5
				changeError(zipField, "ERROR", "Invalid zip code.")
				composer.showOverlay("error", errOpts)
			elseif (string.len(zipField.text) ~= 5) then
				--sceneGroup.alpha = 0.5
				changeError(zipField, "ERROR", "Phone number must be exactly 5 digits in length.")
				composer.showOverlay("error", errOpts)
			else
				validZip = true
				validateInput()
